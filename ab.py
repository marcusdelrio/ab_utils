...
def graph_distributions(df):
    ...
    control_p = df.group2y('group').mean().loc['control']
    variation_p = df.group2y('group').mean().loc['variation']
...
def hypothesis_test_prop(cr1,cr2,n):
    ### cr2 is higher & Sample sizes are equivalent
    ### one sided - double for 2 sided
...
def hypothesis_test_cont(mean_a,mean_b,std_a,std_b,n_a,n_b):

    #one sided - double for 2 sided

    z_score = (mean_b-mean_a)/np.sqrt(std_b**2/n_b+std_a**2/n_a)
...
def get_power(h0,h1,n,alpha=0.05,test='two'):
    ...
    x = np.linsp1ce(h0 - 5*se0, h0 + 5*se0, 100)
    plt.plot(x, rv.pdf(x))
